...
**First-use (locked):** do **not** copy a pre-seeded **database** (`pg0` instance data) out of the bundle. Create an **empty** writable data root at `~/.openbot/hindsight/data` (`mkdir -p`). The bundle stays **read-only** for python + baked weights + `bin/hindsight-api` + vendored **`pg0-installation/`** (PostgreSQL binaries only — not instance data). Spawn remaps the child `HOME` to that data root and sets `HINDSIGHT_API_DATABASE_URL=pg0://hindsight` so pg0’s `~/.pg0` lands under `hindsight/data`. Before start, seed `HOME/.pg0/installation` from bundled `pg0-installation/` (no GitHub download). Missing binaries → visible `memory-error` (`hindsight-pg-missing`), never silent online fetch. Set `HF_HOME` to the **bundled** `hf-cache`. Env always includes `HF_HUB_OFFLINE=1` and `TRANSFORMERS_OFFLINE=1`. Local bake without full rebundle: `cp -R ~/.pg0/installation resources/hindsight/pg0-installation` (no credentials).
...
- Shared Codex login for Hindsight LLM: `CODEX_HOME=~/.openbot/codex-home` (same as harness login; when tests set `OPENBOT_HOME`, `join(home,'codex-home')`). Do **not** point at empty `hindsight/codex`.
- Deny leftover absolute `~/.pg0` (real `os.homedir()/.pg0`) in Claude write-deny and Codex permission profiles in addition to `~/.openbot/hindsight`.
...
- **Codex argv** (binary `codex` on PATH; env `CODEX_HOME` = `~/.openbot/private/<slug>/codex-home`; cwd = `workspace/` for main/resume/stop). Verified this machine **2026-08-14**, `codex-cli 0.147.0`. **Main turns, ask cards, and resume** use **`codex app-server`** via `buildAppServerArgv` / `runCodexTurn` (`packages/daemon/src/codex/exec-argv.ts`, `adapter.ts`) — **not** `codex exec --json` — because exec rejects `request_user_input` (`probe2-REVISE-TO-APP-SERVER.md`; matches §3.5). Spawn with `{ detached: true, stdio: ['pipe','pipe','pipe'] }` so JSON-RPC stdin stays open for ask answers and the child is its own process-group leader. `codex.test.ts` asserts `detached: true` and `stdio[0] === 'pipe'`. App-server argv is **`app-server --listen stdio:// --strict-config`** plus optional `-c model_reasoning_effort=<effort>` when `AgentConfig.effort` is set. Model is passed in JSON-RPC `thread/start` / `thread/resume` / `turn/start` params (not `--model` on the app-server argv). **`--strict-config`** errors on unrecognized `config.toml` keys — required so permission profiles do not fail open. Context7 `/openai/codex` (config_toml.rs / cli docs): effort is config key `model_reasoning_effort`. **Never** pass `--sandbox` / `--sandbox workspace-write` — the permission profile in `config.toml` is the gate. If the CLI rejects that `-c` key, **stop and revise**. Do **not** pass `--effort`, `--ask-for-approval`, or `--dangerously-bypass-approvals-and-sandbox`. **Compact only** stays on `codex exec` (below). Do **not** change the working adapter back to exec for main/ask/resume.
  - First turn / ask / resume (same process): `codex app-server --listen stdio:// --strict-config [-c model_reasoning_effort=<effort>]`, then JSON-RPC `initialize` → `thread/start` (no session) or `thread/resume` `{ threadId, model }` (has session) → `turn/start` with the prompt. Ask answers are JSON-RPC responses to `item/tool/requestUserInput` on that same stdin (fixtures below). **Do not** use `codex exec` for main, ask, or resume.
  - Stop: `process.kill(-child.pid, 'SIGTERM')` then `process.kill(-child.pid, 'SIGKILL')` after 5s. `inFlightPid` is `child.pid`.
  - Compact one-shot: `codex exec "<msg>" --json --strict-config --dangerously-bypass-hook-trust --skip-git-repo-check [--model <model>] [-c model_reasoning_effort=<effort>]` (no `resume`). Memory snapshot no longer uses Codex.
  - `codex.test.ts` asserts `buildAppServerArgv()` equals `['app-server', '--listen', 'stdio://', '--strict-config']`; argv contains **no** `--sandbox` / `--effort`; **contains** `--strict-config`; when effort is set, argv contains `-c` `model_reasoning_effort=<effort>`; compact exec asserts `--strict-config` and optional `--model` / effort `-c`.
...
- Default Codex model `gpt-5.6-luna`. Main/ask/resume argv is `buildAppServerArgv` (`app-server --listen stdio:// --strict-config` plus `-c model_reasoning_effort=<effort>` when effort is set); model goes in JSON-RPC `thread/start` / `thread/resume` / `turn/start`, not `--model` on app-server argv (§3.5 / §8 Codex argv). Compact exec may pass `--model`. If the CLI rejects the effort `-c` key, **stop and revise**. Do not pass `--effort`, `--plan`, or `--sandbox`. No Plan mode / collaboration_modes.
